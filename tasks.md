...
- [x] **T-088** 📥 Fix the **parser against the real source file** — ad-hoc QA caught the importer rejecting `TRACKER -- KDL.xlsx` with 545 errors / 0 parsed ("Data row before any year separator or header row"). The parser (T-060) was coded to D-036's idealized structure; the real file differs. Done 2026-05-31.
  - Root causes (all confirmed by byte-level inspection of the workbook): (1) year separator is a **merged banner** repeating the year across ~20 cols, not a single cell; (2) the **header precedes** the first year banner; (3) **one header** is shared across both year sections; (4) the **container-type column has no header** (merged into "No. of Cont(s)"); (5) header typos `CURENT STATUS` / `TANESWS Loadging` / `TBS Loadging` / `Inspectione file` / `B/L No;`.
  - Fix (`src/server/import/parse-tracker.ts`): `detectYearSeparator` now matches a row where every non-empty cell is the same 4-digit year; header discovery is independent of year state and the map is **sticky** (a year banner no longer clears it); pre-header junk above the first year is skipped (not errored) while an unplaceable data row inside a year section still errors; `tryBuildHeaderMap` adds a strict container-type fallback (column right of the count column); `normaliseHeader` strips `;`; typo aliases added.
  - D-047 logged (supersedes D-036's year-detection + ordering assumptions; two-bucket output + header-driven philosophy + D-035 purity unchanged).
  - Tests: +6 synthetic cases in `tests/unit/parse-tracker.test.ts` (full-width year banner, header-before-year, two sections / one header, header-less container-type inference, title-junk skip, header typos). Real-file smoke (throwaway script, not committed): **508 parsed, 0 structural errors, years [2025, 2026]** (274 + 234); the 4 remaining errors are a misaligned refund-notes appendix below the TOTAL row with no ref_no — correctly flagged, not imported.
  - Gates: typecheck clean, lint 0 errors / 5 pre-existing warnings, 73/73 unit tests.
- [x] **T-089** 📁 **Folder-based file system per consignment** (D-055) — user-requested, evolves the flat attachments feature into a free-form folder tree. Done 2026-06-22.

- [~] **T-090** 🎨 **Excel-style desktop Pipeline Matrix** (D-074) — user-requested. The code, unit suite, typecheck, lint, and production build are complete. Manual authenticated browser QA remains in **H-015** for filters, responsive tabs, and a real guarded stage advance; do not mark done until that is recorded.
  - Migration `20260622220000_consignment_folders.sql`: new `consignment_folders` table (self-ref `parent_folder_id`, RLS mirroring `attachments`, audit + `set_updated_at` triggers, soft-delete, unique live-name-per-parent index with a COALESCE sentinel so root siblings are covered); `attachments.folder_id` FK (`on delete set null`, NULL = root, no backfill); widened the bucket `allowed_mime_types` from images+PDF to also include txt / doc / docx / xls / xlsx. Applied to `kdl-tracker-dev` via MCP `apply_migration`.
  - Schemas: extended `ALLOWED_ATTACHMENT_MIME` + `ATTACHMENT_ACCEPT` (added extensions for Office files whose `file.type` is often empty); `recordAttachmentSchema` gained nullable `folderId`. New `src/schemas/folder.ts` (create/rename/delete/move, name normalization, slash-ban).
  - Server actions: new `folder-actions.ts` (`createFolderAction`, `renameFolderAction`, `deleteFolderAction` — recursive subtree soft-delete + best-effort byte cleanup) + extended `attachment-actions.ts` (`recordAttachmentAction` validates folder belongs to consignment; new `moveAttachmentAction`). Permissions mirror the existing model: create/upload = operator+admin, rename/delete/move = admin.
  - UI: `attachments-tab.tsx` rebuilt in place as a folder browser — breadcrumb nav, folder grid, inline new-folder + rename editors, upload-to-current-folder, admin move (dropdown) / delete. `page.tsx` + `consignment-detail.tsx` thread folders through. Word/Excel download rather than preview (expected).
  - Gates: typecheck clean (only pre-existing missing-module errors for canvas-confetti/@react-pdf/exceljs, none in touched files), lint 0 errors in touched files, Supabase security advisors show no new findings for `consignment_folders`.
...
- [x] **T-072** 📊 Export to PDF (@react-pdf/renderer) for every report. Done 2026-05-31.
  - Accept: PDF opens, has Kingdao logo header, paginated correctly. ✓ Each of the six reports on `/reports` exposes an **Export PDF** anchor next to the XLSX anchor (`<a href download>`, GET-based). Route handler at `src/app/api/reports/[kind]/pdf/route.ts` (`runtime: "nodejs"`, `dynamic: "force-dynamic"`) mirrors the XLSX route: validates `kind` against `REPORT_OPTIONS`, gates on `getServerPermissions()` (401 unauth; viewers + up), parses `year`/`from`/`to` with the same ISO-date validator, runs the shared `fetchReportRows()`, then `buildReportPdf()` → `renderToBuffer()` → `Response` with `application/pdf` + `Content-Disposition: attachment; filename="kdl-<kind>-<year>[-<from>-<to>].pdf"`.
  - Built: `src/server/reports/build-pdf.tsx` (pure react-pdf element builder, mirrors `build-xlsx.ts`). A4 landscape; fixed page header with the Kingdao logo (`public/KINGDAO_LOGO.png`, loaded once via `readFileSync(process.cwd()/KINGDAO_LOGO.png)`, blank-box fallback if missing) + title + `Generated … UTC` + filter summary, repeated on every page via react-pdf `fixed`; footer with `page / total` counter. Per-report table component switched on `payload.kind`; money via `formatTzs`, dates `yyyy-mm-dd`, totals row matching the page/XLSX on Revenue / Client Volume / Pipeline Bottleneck / Pending Refunds; empty result renders `"No rows matched the filter."`; header + table header repeat on page breaks, data rows `wrap={false}`.
  - Reused: `fetchReportRows()` + `reportTitle()` + `reportFilenameStem()` + `REPORT_OPTIONS` from the T-071 seam (page, XLSX route, and PDF route all read identical data).
  - Tests: `tests/unit/build-pdf.test.ts` — 9 cases (each of the six kinds renders a valid `%PDF` buffer; revenue/pending-refunds empty + null-funnel payloads render without throwing). Total Vitest count 67/67.
  - D-026: no admin-client uses in any new file (allowlist stays at 3 sites). D-035: no SheetJS — `build-pdf.tsx` imports only `@react-pdf/renderer` + `@/lib/money` + `./report-types`.
  - Side fixes folded in: 6× `ReportHeader` calls now pass the required `exportHrefPdf` prop; dropped the stale "PDF export is tracked as T-072" footer note from `page.tsx`; one `jsx-a11y/alt-text` false positive on react-pdf's `<Image>` suppressed with a targeted disable + comment.
  - V-REPORTS-PDF added to `validation.md`. Gates: typecheck clean, lint 0 errors / 5 pre-existing warnings, 67/67 unit tests, `pnpm build` clean with `/api/reports/[kind]/pdf` in the route manifest.
