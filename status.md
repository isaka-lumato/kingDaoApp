...
| Field | Value |
|---|---|
| **Phase** | 4 — in progress. T-050, T-051, T-052 shipped. Next: T-053 (alerts edge function) or T-054 (dashboard). |
| **Last updated** | 2026-05-23 |
| **Last task completed** | T-052 (GUTA pair linkage UI on consignment detail — sibling card + asymmetric-release warning per PRD §8.15) |
| **Current task in progress** | None — ready to pick up T-053 (alerts) or T-054 (dashboard). |
| **Blocked tasks** | None |
...
1. **T-053** — Alerts edge function (Resend + Supabase scheduled) — newly-stuck stages email admins every 30 min. Unblocked now that T-049 has set a clean latency baseline.
2. **T-054** — Dashboard with active jobs, pipeline funnel, arrivals this week, revenue, top clients, overdue jobs. The "Released but no EFD" flag is now feasible to surface (PRD §8.14) since EFD records have first-class link state.
3. **Ad-hoc QA + follow-ups** — operator + admin click-through of `/efd`, `/consignments` (batch drawer), and the GUTA pair section end-to-end against the dev DB (V-EFD + V-BATCH + V-GUTA checks). Verify the batch auto-link behaves correctly when a sibling is soft-deleted mid-link, and that GUTA pair section hides cleanly when the sibling is soft-deleted. Plus the residual @dnd-kit hydration warning on the kanban — try `useId()` or `next/dynamic({ ssr: false })`.
