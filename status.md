...
| Field | Value |
|---|---|
| **Phase** | 4 — in progress. T-050, T-051, T-052, T-054 shipped + T-055 hotfix. Next: T-053 (alerts edge function). |
| **Last updated** | 2026-05-23 |
| **Last task completed** | T-055 (Hotfix: pre-existing T-042 filter bugs on `/consignments` — `unreleased` crashed with bad enum literal; `stuck` returned wrong set. Surfaced by T-054 dashboard deep-links.) |
| **Current task in progress** | None — ready to pick up T-053 (alerts edge function). |
| **Blocked tasks** | None |
