...
| Field | Value |
|---|---|
| **Phase** | 4 mostly done + Phase 5 in progress. T-060 + T-061 shipped 2026-05-24. **T-053b blocked on H-004 + H-010** (Resend account + edge-function deploy). Next: T-062 (CLI importer for bulk historical load) + ad-hoc QA of the import flow against the real `TRACKER -- KDL.xlsx`. |
| **Last updated** | 2026-05-24 |
| **Last task completed** | T-061 (Excel import UI — `/import` with SheetJS adapter, preview/confirm flow, row-by-row commit, auto-create missing clients/ICDs, `import_jobs` audit table). |
| **Current task in progress** | None — T-053b blocked on H-010; T-062 (CLI importer) is the next codeable task. |
| **Blocked tasks** | None |
...
2. **Ad-hoc QA of `/import` against the real tracker.** Upload `fixtures/TRACKER -- KDL.xlsx` as admin → preview should show the years present + parsed/errors/warnings counts. Spot-check the auto-create chips against PRD §13.1/§13.2 (expect zero unknown clients/ICDs for the canonical tracker). Confirm import in a fresh year (or use dev project state intentionally) and verify rows appear on `/consignments`, `/dashboard` revenue tile, and the kanban.
3. **T-062** — CLI importer (`scripts/import-tracker.ts`). Same parser, server-side script; designed for the initial bulk historical load. Reuses `workbookToRows` + the FK-resolution + insert logic from `import-actions.ts` — extract to a shared `commit-rows.ts` helper if duplication starts to bite.
4. **Released-but-no-EFD widget** — bolt onto `/dashboard` as a 7th panel once we agree the surfacing model (PRD §8.14). Now feasible since EFD records have first-class link state.
5. **Ad-hoc QA + follow-ups** — operator + admin click-through of `/dashboard`, `/efd`, `/consignments` (batch drawer), and the GUTA pair section end-to-end against the dev DB (V-DASH + V-EFD + V-BATCH + V-GUTA checks). Verify the batch auto-link behaves correctly when a sibling is soft-deleted mid-link, and that GUTA pair section hides cleanly when the sibling is soft-deleted.
