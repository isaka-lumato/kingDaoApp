...
export default function HomeShell({ icds, ...props }: Props) {
  ...
  // Explicit user override of the auto-default. Null = follow viewport (Matrix on desktop, Triage on mobile).
  const [override, setOverride] = useState<Tab | null>(null);
  const tab: Tab = override ?? (isDesktop ? "matrix" : "triage");
  const setTab = (t: Tab) => setOverride(t);
  const router = useRouter();

  // Live updates (D-057). The matrix/kanban/triage views are RSC-props + useOptimistic,
  ...
  // refresh; the actor's own change is already reflected optimistically.
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      ...
      <div className="flex-1 min-h-0">
        {tab === "matrix" ? (
          <PipelineMatrix {...props} icds={icds} />
        ) : tab === "kanban" ? (
