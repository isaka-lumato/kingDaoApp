...
type ModalState = { mode: "add" } | { mode: "edit"; row: IcdTableRow } | null;

type SortKey = "name" | "location" | "consignmentCount";
type SortDir = "asc" | "desc";

export default function IcdsTable({
  icds,
  isAdmin,
}: {
  icds: IcdTableRow[];
  ...
  const rows = useMemo(() => {
    ...
    const filtered = q
      ? icds.filter((i) =>
          [i.name, i.location].filter(Boolean).some((v) => (v as string).toLowerCase().includes(q)),
        )
      : icds;

    const sorted = [...filtered].sort((a, b) => {
      let cmp: number;
      if (sort === "consignmentCount") {
        cmp = a.consignmentCount - b.consignmentCount;
      } else {
    ...
    return sorted;
  }, [icds, search, sort, dir]);

  function openAdd() {
    setFormError(null);
    setModal({ mode: "add" });
  }
  function openEdit(row: IcdTableRow) {
    setFormError(null);
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const action = modal.mode === "add" ? createIcdAction : updateIcdAction;
  ...
  function toggleActive(row: IcdTableRow) {
    setTogglingId(row.id);
    const fd = new FormData();
    fd.set("id", row.id);
    fd.set("isActive", String(!row.is_active));
    startTransition(async () => {
      await setIcdActiveAction(fd);
      setTogglingId(null);
      router.refresh();
    });
  }

  function handleDelete(row: IcdTableRow) {
    if (
      !window.confirm(
        `Delete ICD "${row.name}"? This can't be undone from the UI.`,
      )
    )
      return;
    const fd = new FormData();
    fd.set("id", row.id);
    startTransition(async () => {
      const res = await deleteIcdAction(fd);
      if (res && "error" in res && res.error) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-foreground tracking-tight">ICDs</h1>
          <p className="text-muted-foreground text-sm mt-0.5">
            {icds.length.toLocaleString()} depot{icds.length !== 1 ? "s" : ""}
          </p>
        </div>
        {isAdmin && (
          <button
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-muted/40">
                ...
                <SortHeader column="location" label="Location" activeSort={sort} activeDir={dir} onSort={onSort} align="left" />
            ...
            <tbody className="divide-y divide-border">
              ...
              {rows.map((row) => (
                <tr
                  ...
                  onClick={() => router.push(`/icds/${row.id}`)}
                >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative z-10 w-full max-w-sm rounded-2xl border border-border bg-card p-6 shadow-2xl">
            <h3 className="text-lg font-semibold text-foreground mb-1">
              {modal.mode === "add" ? "New ICD" : "Edit ICD"}
            </h3>
            <p className="text-muted-foreground text-sm mb-5">
              {modal.mode === "add"
                ? "Adds an Inland Container Depot / yard."
                : "Update this ICD's details."}
            </p>
            ...
            <form key={modal.mode === "edit" ? modal.row.id : "add"} onSubmit={handleSubmit} className="space-y-4">
              ...
              <ModalField
                ...
                defaultValue={modal.mode === "edit" ? (modal.row.location ?? "") : ""}
              />
              <div className="flex gap-3 pt-1">
                ...
                >
                  {isPending ? "Saving…" : modal.mode === "add" ? "Create ICD" : "Save changes"}
                </button>
              </div>
            </form>
          </div>
...
function ModalField({
  ...
  return (
    <div className="space-y-1.5">
      <label htmlFor={`icd-field-${name}`} className="block text-sm font-medium text-foreground">
        {label}
